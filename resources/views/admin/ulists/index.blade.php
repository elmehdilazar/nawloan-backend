@extends('layouts.admin.app')
@section('title',' | ' . __('site.ulists'))
@section('styles')
    <!-- DataTable CSS -->
    <link rel="stylesheet" href="{{asset('assets/tiny/css/dataTables.bootstrap4.css')}}">
@endsection
@section('content')
    <div class="flex-space mb-4 dash-head">
        <h2 class="section-title mb-0">@lang('site.ulists')</h2>
        <div class="head-btns mb-0">
            <span id="checks-count" class="onchange-visible"></span>
            @if(auth()->user()->hasPermission('ulists_export'))
                <a class="btn btn-transparent navy" href="{{route('admin.ulists.export')}}"
                   title="@lang('site.export') @lang('site.ulists')">
                    @lang('site.export')
                </a>
            @endif
            @if(auth()->user()->hasPermission('ulists_disable'))
                <a href="#" id="bulk-delete" class="btn btn-danger onchange-visible">@lang('site.delete_selected')</a>
            @endif
            @if(auth()->user()->hasPermission('ulists_create'))
                <a class="btn btn-navy onchange-hidden" href="{{route('admin.ulists.create')}}"
                   title="@lang('site.add') @lang('site.ulist')">
                    @lang('site.add') @lang('site.ulist')
                </a>
            @endif
        </div>
    </div>
    <table class="table datatables datatables-active" id="">
        <thead>
            <tr>
                <th>
                    <div class="dt-checkbox">
                        <input type="checkbox" name="select_all" value="1" id="selectAll">
                        <label for="selectAll" class="visual-checkbox"></label>
                    </div>
                </th>
                <th>@lang('site.num')</th>
                <th>@lang('site.name_en')</th>
                <th>@lang('site.name_ar')</th>
                <th>@lang('site.users')</th>
                <th>@lang('site.status')</th>
                <th>@lang('site.edit')</th>
            </tr>
        </thead>
        <tbody>
        @foreach ($ulists as $index=>$list)
            <tr>
                <td>{{$list->id}}</td>
                <td>{{$index + 1}}</td>
                <td>{{$list->name_en}}</td>
                <td class="text-arabic">{{$list->name_ar}}</td>
                <td>
                    <a href="#" data-toggle="modal" data-target="#showModal_{{$index}}" title="@lang('site.view')">
                        {{$list->users->count()}}</td>
                    </a>
                <td>
                    <span class="badge badge-pill {{$list->active==1 ? 'badge-primary ': 'badge-danger'}}">
                        {{$list->getActive()}}
                    </span>
                </td>
                <td>
                    <ul class="actions">
                        @if(auth()->user()->hasPermission('ulists_update'))
                            <li>
                                <a href="{{route('admin.ulists.edit',$list->id)}}" class="show" title="@lang('site.edit')">
                                    <i class="fad fa-edit"></i>
                                </a>
                            </li>
                        @endif
                        @if(auth()->user()->hasPermission('ulists_disable') || auth()->user()->hasPermission('ulists_enable'))
                            @if($list->active==1)
                                <li>
                                    <a class="cancel" href="#" data-toggle="modal" data-target="#enableModal_{{$index}}" title="@lang('site.disable')">
                                        <i class="fad fa-times"></i>
                                    </a>
                                </li>
                            @else
                                <li>
                                    <a class="success" href="#" data-toggle="modal" data-target="#enableModal_{{$index}}" title="@lang('site.enable')">
                                        <i class="fad fa-check-double"></i>
                                    </a>
                                </li>
                            @endif
                        @endif
                    </ul>
                </td>
            </tr>
        @endforeach
        </tbody>
    </table>
    <div class="d-flex justify-content-start">
        {{$ulists->appends(request()->query())->links()}}
    </div>
    @foreach ($ulists as $index=>$list)
        <div class="modal fade mini-modal" id="enableModal_{{$index}}" tabindex="-1" role="dialog"
             aria-labelledby="verticalModalTitle_{{$index}}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="verticalModalTitle_{{$index}}">
                            @if($list->active==0)
                                @lang('site.enable') @lang('site.ulist')
                            @else
                                @lang('site.disable') @lang('site.ulist')
                            @endif</h5>
                        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close">
                            <svg xmlns="http://www.w3.org/2000/svg" width="38" height="38" viewBox="0 0 38 38">
                                <path id="Exclusion_23" data-name="Exclusion 23"
                                      d="M26.384,37.578h-14a12,12,0,0,1-12-12v-14a12,12,0,0,1,12-12h14a12,12,0,0,1,12,12v14a12,12,0,0,1-12,12Zm-7-16.4h0L26,27.793l2.6-2.6-6.617-6.617L28.6,11.961,26,9.363,19.384,15.98,12.767,9.363l-2.6,2.6,6.617,6.617-6.617,6.617,2.6,2.6,6.616-6.616Z"
                                      transform="translate(-0.384 0.422)" fill="#d27979" />
                            </svg>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p class="text-center">
                            @if ($list->active==0)
                                @lang('site.enable_confirm') @lang('site.ulist') @lang('site.question_mark')
                            @else
                                @lang('site.disable_confirm') @lang('site.ulist') @lang('site.question_mark')
                            @endif
                        </p>
                        <form action="{{route('admin.ulists.changeStatus',$list->id)}}" method="post">
                            @csrf
                            @method('post')
                            <div class="d-flex justify-content-center mt-3 gap-15">
                                <button type="submit" class="btn shadow-none @if($list->active==0) btn-navy @else btn-danger @endif"
                                        title="@if($list->active==0)@lang('site.enable')@else @lang('site.disable')@endif">
                                    {{--<i class="fa @if($list->active==0) fa-check @else fa-remove @endif"></i>--}}
                                    @if($list->active==0)
                                        @lang('site.enable')
                                    @else
                                        @lang('site.disable')
                                    @endif
                                </button>
                                <button type="submit" class="btn shadow-none btn-transparent navy" title="@lang('site.cancel')"
                                        data-dismiss="modal" aria-label="Close">
                                    {{--<i class="fas fa-remove"></i> --}}
                                    @lang('site.cancel')
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
        <div class="modal fade mini-modal" id="showModal_{{$index}}" tabindex="-1" role="dialog"
             aria-labelledby="verticalModalTitle1_{{$index}}" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="verticalModalTitle1_{{$index}}">
                            {{app()->getLocale()=='ar' ? $list->name_ar : $list->name_en}}
                        </h5>
                        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close">
                            <svg xmlns="http://www.w3.org/2000/svg" width="38" height="38" viewBox="0 0 38 38">
                                <path id="Exclusion_23" data-name="Exclusion 23"
                                      d="M26.384,37.578h-14a12,12,0,0,1-12-12v-14a12,12,0,0,1,12-12h14a12,12,0,0,1,12,12v14a12,12,0,0,1-12,12Zm-7-16.4h0L26,27.793l2.6-2.6-6.617-6.617L28.6,11.961,26,9.363,19.384,15.98,12.767,9.363l-2.6,2.6,6.617,6.617-6.617,6.617,2.6,2.6,6.616-6.616Z"
                                      transform="translate(-0.384 0.422)" fill="#d27979" />
                            </svg>
                        </button>
                    </div>
                    <div class="modal-body">
                        <table class="table datatables datatables-active check-disabled">
                            <thead>
                                <tr>
                                    <th>@lang('site.num')</th>
                                    <th>@lang('site.user')</th>
                                    <th>@lang('site.type')</th>
                                    <th>@lang('site.remove')</th>
                                </tr>
                            </thead>
                            <tbody>
                            @foreach($list->users as $index=>$user)
                                <tr>
                                    <td>{{$index + 1}}</td>
                                    <td>{{$user->user->name}}</td>
                                    <td>{{$user->user->type}}</td>
                                    <td>
                                        <ul class="actions">
                                            <li>
                                                <a href="#" class="cancel">
                                                    <i class="fad fa-trash-alt"></i>
                                                </a>
                                            </li>
                                        </ul>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                        @if($list->users->count()==0)
                        <div class="flex-center">
                            <p style="text-align: center">@lang('site.no_records_found')</p>
                        </div>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    @endforeach
@endsection

@section('scripts')
    <!-- Data Tables -->
    <script src='{{asset('assets/tiny/js/jquery.dataTables.min.js')}}'></script>
    <script src='{{asset('assets/tiny/js/dataTables.bootstrap4.min.js')}}'></script>
    <!-- DataTables Playground (Setups, Options, Actions) -->
    <script src='{{ asset('assets/js/dataTables-init.js') }}?v={{ @filemtime(public_path('assets/js/dataTables-init.js')) }}'></script>
    <script>
        $(document).on('click', '#bulk-delete', function (e) {
            e.preventDefault();
            var selected = [];
            $('.datatables-active tbody input[type="checkbox"]').not('#selectAll').each(function(){
                if($(this).is(':checked')){
                    selected.push($(this).val());
                }
            });
            if(selected.length === 0){
                alert(@json(__('site.no_items_selected')));
                return;
            }
            if(confirm(@json(__('site.delete_selected_confirm')))){
                var url = @json(route('admin.ulists.destroy-selected')) + '?ids=' + selected.join(',');
                window.location = url;
            }
        });
    </script>
@endsection
