...
@section('content')
    <div class="flex-space mb-4 dash-head">
        <h2 class="section-title mb-0">@lang('site.cars')</h2>
        <div class="head-btns mb-0">
            <span id="checks-count" class="onchange-visible"></span>
            @if(auth()->user()->hasPermission('cars_export'))
                <a class="btn btn-transparent navy" href="{{route('admin.trucks.export')}}"
                   title="@lang('site.export') @lang('site.cars')">
                    @lang('site.export')
                </a>
            @endif
            @if(auth()->user()->hasPermission('cars_disable'))
                <a href="#" id="bulk-delete" class="btn btn-danger onchange-visible">@lang('site.delete_selected')</a>
            @endif
            @if(auth()->user()->hasPermission('cars_create'))
                <a class="btn btn-navy onchange-hidden" href="{{route('admin.trucks.create')}}"
                   title="@lang('site.add') @lang('site.car')">
                    @lang('site.add') @lang('site.car')
                </a>
            @endif
        </div>
    </div>
    <form action="{{route('admin.trucks.index')}}" method="GET">
        <div class="row search-by-group mb-5">
            ...
            <div class="col-xl-3 col-lg-4 col-md-6 col-12 mb-3">
                <div class="input-group mb-0">
                    <label for="weight" class="fe-14">@lang('site.weight')</label>
                    <div class="input-group">
                        <input class="fe-14" type="text" name="weight" id="weight" value="{{request()->weight}}"
                               placeholder="@lang('site.weight')">
                        <div class="input-group-prepend">
    ...
    <table class="table datatables datatables-active" id="">
        ...
        @foreach ($trucks as $index=>$truck)
            <tr>
                ...
                <td>{{$index + 1}}</td>
                <td>
                    @if(!empty($truck->image))
                        <img src="{{asset($truck->image)}}" class="truck" alt="{{$truck->name_en}}">
                    @else
                        --
                    @endif
                </td>
                <td>{{$truck->name_en}}</td>
                <td class="text-arabic">{{$truck->name_ar}}</td>
                <td>{{$truck->frames}}</td>
                <td>{{$truck->weight .' '.__('site.ton')}}</td>
                <td>
                    <span class="badge badge-pill {{$truck->active==1 ? 'badge-primary ': 'badge-danger'}}">
                        {{$truck->getActive()}}
                    </span>
                </td>
                <td>
                    <ul class="actions">
                        @if(auth()->user()->hasPermission('cars_update'))
                            <li>
                                <a href="{{route('admin.trucks.edit',$truck->id)}}" class="show" title="@lang('site.edit')">
                                    <i class="fad fa-edit"></i>
                                </a>
                            </li>
                        @endif
                        @if(auth()->user()->hasPermission('cars_disable') || auth()->user()->hasPermission('cars_enable'))
                            @if($truck->active==1)
                                <li>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{route('admin.trucks.changeStatus',$truck->id)}}" method="post">
                        @csrf
                        @method('post')
                        <div class="d-flex justify-content-center mt-3 gap-15">
                            <button type="submit" class="btn shadow-none @if($truck->active==0) btn-navy @else btn-danger @endif"
                                    title="@if($truck->active==0)@lang('site.enable')@else @lang('site.disable')@endif">
    ...
    <script>
        $(document).on('click', '#bulk-delete', function (e) {
            ...
            if(confirm(@json(__('site.delete_selected_confirm')))){
                var url = @json(route('admin.trucks.destroy-selected')) + '?ids=' + selected.join(',');
                window.location = url;
            }
        });
    </script>
