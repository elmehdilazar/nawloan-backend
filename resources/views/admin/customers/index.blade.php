...
@section('content')
    <div class="flex-space flex-wrap gap-15 mb-4 dash-head">
        ...
        <div class="head-btns mb-0">
            ...
            @endif
            @if(auth()->user()->hasPermission('customers_disable'))
    ...
    <table class="table datatables datatables-active" id="">
        ...
        <tbody>
            ...
            <td>
                ...
                @endif
                <li>
                    <a href="{{ route('admin.customers.evaluate', $user->id) }}" class="show" title="@lang('site.evaluates')">
    ...
    <script>
        $(document).on('click', '#bulk-delete', function (e) {
            ...
            if(confirm(@json(__('site.delete_selected_confirm')))){
                var url = @json(route('admin.customers.destroy-selected')) + '?ids=' + selected.join(',');
                window.location = url;
            }
        });
    </script>
