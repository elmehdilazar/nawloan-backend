...
@section('title',' | ' . __('site.shipping_companies'))
@section('styles')
    <!-- DataTable CSS -->
...
@section('content')
    <div class="flex-space flex-wrap gap-15 mb-4 dash-head">
        <h2 class="section-title mb-0">@lang('site.shipping_companies')</h2>
        <div class="head-btns mb-0">
            <span id="checks-count" class="onchange-visible"></span>
            @if(auth()->user()->hasPermission('driverCompanies_export'))
                <a href="{{route('admin.companies.export')}}" class="btn btn-transparent navy"
                   title="@lang('site.export') @lang('site.companies')">
                    @lang('site.export')
                </a>
            @endif
            @if(auth()->user()->hasPermission('driverCompanies_disable'))
                <a href="#" id="bulk-delete" class="btn btn-danger onchange-visible">@lang('site.delete_selected')</a>
            @endif
            @if(auth()->user()->hasPermission('driverCompanies_create'))
                <a href="{{route('admin.companies.create')}}" class="btn btn-navy onchange-hidden"
                   title="@lang('site.add') @lang('site.shipping_company')">
                    @lang('site.add') @lang('site.shipping_company')
                </a>
            @endif
        </div>
    </div>
    ...
    <table class="table datatables datatables-active" id="">
        ...
        <tr>
            ...
            <th>@lang('site.num')</th>
            <th class="min-width-170">@lang('site.company_name')</th>
            <th>@lang('site.phone_n')</th>
            <th>@lang('site.drivers')</th>
        ...
        @foreach ($users as $index=>$user)
            <tr>
                ...
                <td>{{$user->phone}}</td>
                <td>
                    <a href="{{route('admin.drivers.index',['company_id'=> $user->id])}}"
                       title="@lang('site.show') @lang('site.drivers')">
                        @lang('site.drivers')
                    </a>
                </td>
                <td>
                ...
                <td>
                    <ul class="actions">
                        @if(auth()->user()->hasPermission('driverCompanies_update'))
                            <li>
                                <a href="{{route('admin.companies.edit',$user->id)}}" class="show">
                                    <i class="fad fa-edit"></i>
                                </a>
                            </li>
                        @endif
                        @if(auth()->user()->hasPermission('driverCompanies_read'))
                        <li>
                            <a href="{{route('admin.companies.evaluate',$user->id)}}" class="show">
                                <i class="fad fa-star-half"></i>
                            </a>
                        </li>
                        @endif
                        @if(auth()->user()->hasPermission('driverCompanies_disable') || auth()->user()->hasPermission('driverCompanies_enable'))
                            @if($user->active==1)
                                <li>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="verticalModalTitle_{{$index}}">
                            @if($user->active==0)
                                @lang('site.enable') @lang('site.shipping_company')
                            @else
                                @lang('site.disable') @lang('site.shipping_company')
                            @endif
                        </h5>
                    ...
                    <div class="modal-body">
                        <p class="text-center">
                            @if ($user->active==0)
                                @lang('site.enable_confirm') @lang('site.shipping_company') @lang('site.question_mark')
                            @else
                                @lang('site.disable_confirm') @lang('site.shipping_company') @lang('site.question_mark')
                            @endif
                        </p>
                        <form action="{{route('admin.companies.changeStatus',$user->id)}}" method="post">
                            @csrf
                            @method('post')
                            <div class="d-flex justify-content-center mt-3 gap-15">
                                <button type="submit" class="btn shadow-none @if($user->active==0) btn-navy @else btn-danger @endif"
    ...
    <script>
        $(document).on('click', '#bulk-delete', function (e) {
            ...
            if(confirm(@json(__('site.delete_selected_confirm')))){
                var url = @json(route('admin.companies.destroy-selected')) + '?ids=' + selected.join(',');
                window.location = url;
            }
        });
    </script>
